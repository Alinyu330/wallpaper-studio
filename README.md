...
![Version](https://img.shields.io/badge/version-1.9.0-7c5cff)
...
**最新版 v1.9.0 安装包**（Windows 10 / 11 · x64 · 约 200 MB · 内置全格式解码器）：

- 国内加速①：[gh-proxy.com 下载](https://gh-proxy.com/https://github.com/Alinyu330/wallpaper-studio/releases/download/v1.9.0/WallpaperStudio-Setup-1.9.0.exe)
- 国内加速②：[ghfast.top 下载](https://ghfast.top/https://github.com/Alinyu330/wallpaper-studio/releases/download/v1.9.0/WallpaperStudio-Setup-1.9.0.exe)
...
- **性能档位与 GPU / 帧率调节**：省电 / 均衡 / 性能三档一键设定音律帧率、组件刷新间隔、硬解模式、视频帧率上限与解码缓存；细分项可单独微调（固定调节点 + 自由输入）；Chromium 硬件加速可关闭（老显卡 / 驱动异常更稳，需重启）；GPU 进程反复崩溃自动降级软件渲染
- **信息看板组件**：日历日程 / 纪念日倒计时 + Open-Meteo 实时与七日天气预报 + 桌面可直接勾选的待办清单
- **系统状态监控组件**：上下行网速实时曲线 + CPU / GPU / 内存占用概览，一屏掌握系统状态
- **组件玻璃质感与吸附**：毛玻璃 / 液态玻璃样式、四种形状、亮度 / 对比 / 饱和 / 不透明度全局调色；拖动时靠近其它组件自动吸附对齐
- **转盘收纳「隐藏到壁纸后」**：第二种收纳方式 — 文件留在原地、仅把桌面图标停到屏幕外（肉眼不可见、无法点选 / 框选），恢复后精确回到原位；回收站 / 此电脑等系统图标恒走此方式；看门狗自动对抗资源管理器重排
- **音律动效全面增强**：新增霓虹样式与亮度 / 对比 / 饱和 / 倒影强度调节；镜面、水面两种新倒影形态
...
- 修复组件面板不透明度滑杆无效的问题
- 修复音量组件拖动调节无效果的问题
- 修复部分快捷方式在转盘显示空白文件图标的问题
- 修复音律动效镜像倒影不同步、残影 / 撕裂的问题（绘制全面重写）
- 音律动效在窗口最大化 / 应用全屏时自动暂停绘制，省 GPU 不空转
...
- **桌面 DIY 组件**（v1.2.0，v1.7.0 重构，v1.9.0 增强）：时钟（12/24 小时制点击切换）、信息看板（日历 / 天气 / 待办）、系统状态监控（网速 / CPU / GPU / 内存）、音量控制条、音律动效 — 每个组件独立小窗口，无边框融入壁纸；毛玻璃 / 液态玻璃样式与全局调色；新增「调整模式」：客户端一键进入拖动调整、桌面默认只显示不误触；组件 / 音律动效 / 转盘均支持九宫格快速定位与拖动吸附
